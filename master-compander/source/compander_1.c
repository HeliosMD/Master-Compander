// Compress or expands the wavefrom of an audio file
...
int main(int argc, char *argv[])
{
    ...
    printf("COMPANDER_1 (Processing - 64bit floating point)\n");
	...
	if(audio_buffer < 0)
	{
		double base = abs(audio_buffer) / NORM;
		double sample = -1 * NORM * pow(base, exp);
		audio_buffer = round(sample);
	}
	else
	{
		double base = audio_buffer / NORM;
		double sample = NORM * pow(base, exp);
		audio_buffer = round(sample);
	}
